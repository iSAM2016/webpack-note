...
// console.log(chalk.cyan(`scp -r ./${config.jsonPath}/web/*   suhailong@dev01.bj.lejent.cc:/data/static/${config.jsonPath}/\n`));

// shell.exec(`scp -r ./${config.jsonPath}/web/*   suhailong@dev01.bj.lejent.cc:/data/static/${config.jsonPath}/`);
// 'scp -r ./wb/fudao/m/180118/web/*  suhailong@dev01.bj.lejent.cc:/data/static/wb/fudao/m/180118/'
...
async function updateFile() {
    ...
    console.log(chalk.green(`本地文件`, path.join(__dirname, `../${config.jsonPath}` + '/web', `服务器`, global.assetsRoot)))
    // 上传文件夹
    let status = await ssh.putDirectory(path.join(__dirname, `../${config.jsonPath}` + '/web'), `${global.assetsRoot}`, {
        // 递归
        recursive: true,
        // 并发数
        concurrency: 10,
